...
![alt text](res/1761992341269_image.png)

The bad thing is that these experiments take days, so iterations speed is very small. That makes difficult
...
Training on a bigger dataset does not prevent the collapse. Neither does using a bigger number of generations per step.
...
- [x] Could the random RAM problems be caused by evaluating the code generated by the LLM?
- [x] Is the model improving when training on BARC data?
